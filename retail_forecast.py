...
def load_data_from_csv(file_path='static/Price_Elasticity_Frozen_Input.csv', sample_size=0.05, random_state=42):
    ...
    try:
        ...
        # Use Category_Name as the Product
        df['Product'] = df['Category_Name']
        ...
        columns_to_drop = ['Store_Id', 'proc_date', 'Category_Name', 'Item']
        for col in columns_to_drop:
            if col in df.columns:
                df.drop(col, axis=1, inplace=True)
        
        print(f"Loaded {len(df)} records for {len(df['Product'].unique())} products")
        return df
...
def generate_sample_data():
    """Generate sample historical sales and inventory data for multiple products"""
    
    print("Falling back to sample data generation...")
    
    # Define products
    products = ['Potatoes', 'Vegetables', 'Baby Products', 'Fruit', 'Milk', 'DVDs', 'Meat']
